...
function updateVideo(id){
	if ($("#oursGrid").datagrid('getSelections').length == 1) {
		updateVideoAble.call(this, $("#oursGrid").datagrid('getSelected').id);
	} else {
...
function deleteVideo(id){
	if ($("#oursGrid").datagrid('getSelections').length == 1) {
		parent.simpleMessConf.call(this,
				 ...
				 function(result){
					if(result){
						deleteVideoAble.call(this, $("#oursGrid").datagrid('getSelected').id);
					}
				}
		);
	} else {
...
function deleteVideoAble(id){
	$.post('${cy}/video/videoDelete/'+id+'.do',"queryType=simpleDelete",function(json){
		if (json.success) {
			$("#oursGrid").datagrid("deleteRow",
				 $("#oursGrid").datagrid("getRowIndex",id)
			);
		}
		parent.simpleMessAlert.call(this,'提示',json.message);
	},'json');
...
function updateVideoAble(id){
	mainDialog = parent.cy.dialog({
		...
		buttons : [ {
			...
			handler : function() {			
				...
				$.post('${cy}/video/videoMerge.do',cy.serializeObject(formDia),function(json){
					if (json.success) {
						$("#oursGrid").datagrid("updateRow",{
							index : $("#oursGrid").datagrid("getRowIndex",id) ,
							row : json.result
						});
						mainDialog.dialog('close');
					}
		...
		onLoad : function(){
			formDia = mainDialog.find('form');
			formDia.form('load',$("#oursGrid").datagrid("getSelected"));	
		}
