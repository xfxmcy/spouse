 ...
 *            回调方法
 */
sy.messagerConfirm = function(ob) {
	
	return $.messager.confirm(ob.title, ob.msg, ob.fn);
};
 ...
 * @Date  12/08/2013
 * 
 * 表格选中提示,无选中或多选中返回 错误信息 ,选中一个返回  null 
 */
sy.singleSelected = function(dg) {
	var con = dg.datagrid("getChecked");
	
	
	if(con.length==0 || con.length>1 ){
		return  "请您选中一条记录,在进行操作";
	}
	if(con.length ==1 ){
		return  null;
	}
 ...
 * 表格选中提示,无选中或多选中返回 错误信息 ,选中一个返回  null 
 */
sy.singleSelectedInTree = function(dg) {
	var con = dg.datagrid("getChecked");
	console.info(con);
 ...
 * 弹出层 
 */
sy.showParentMessage = function(msg,title) {
