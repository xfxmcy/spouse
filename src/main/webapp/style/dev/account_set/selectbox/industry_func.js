...
var Industry = {
	...
	confirm : function(){
		...
		indStr=indStr.substring(1)?indStr.substring(1):'请选择技能';
...
function IndustrySelect(){
	var dragHtml ='<div id="IndustryAlpha">';		//技能
		dragHtml+='		<dl id="IndustrySelected"><dt>已选技能：</dt><dd></dd></dl>';
		dragHtml+='		<div id="IndustryList"></div>';//技能列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择技能（您最多能选择5项）</b><span onclick="Industry.confirm()">确定</span>');
...
function IndustrySelect_2(){
	var dragHtml ='<div id="IndustryAlpha">';		//技能
		dragHtml+='		<div id="IndustryList" class="radio"></div>';//技能列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择技能</b><span onclick="boxAlpha()">关闭</span>');
	$('#drag_con').html(dragHtml);
